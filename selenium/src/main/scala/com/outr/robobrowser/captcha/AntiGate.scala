...
object AntiGate {
  def apply(key: String,
            ...
            checkDelay: FiniteDuration = 3.seconds): Task[Either[AntiGateError, AntiGateStatus]] = {
    create(key, url, template, proxy, variables).flatMap {
      case Left(error) => Task.pure(Left(error))
  ...
  private def create(key: String,
                     ...
                     template: String,
                     proxy: Proxy,
                     variables: Map[String, String]): Task[Either[AntiGateError, AntiGateTask]] = {
  ...
  private def status(task: AntiGateTask, proxy: Proxy): Task[AntiGateStatus] = {
  ...
  private def result(task: AntiGateTask,
                     proxy: Proxy,
                     initialDelay: FiniteDuration = 5.seconds,
                     checkDelay: FiniteDuration = 3.seconds): Task[AntiGateStatus] = {
    val init = Task.sleep(initialDelay)
    init.flatMap { _ =>
      status(task, proxy).flatMap { status =>
        if (status.status.contains("processing")) {
          Task.sleep(checkDelay).flatMap { _ =>
