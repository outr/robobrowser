...
case class BrowserStack(browser: RoboBrowser) extends AnyVal {
  ...
  def markAsync(status: BrowserStack.Status): Task[Json] =
    BrowserStack.mark(browser.sessionId, options.username, options.automateKey, status)

  def mark(status: BrowserStack.Status): Json = markAsync(status).sync()
}
...
object BrowserStack {
  ...
  private def toJson(response: HttpResponse): Json = {
    ...
    JsonParser(content.asString.sync())
  }

  def status(sessionId: String, username: String, automateKey: String): Task[Json] = client(sessionId, username, automateKey)
    .get
    .send()
    .map(toJson)

  def mark(sessionId: String, username: String, automateKey: String, status: Status): Task[Json] = {
