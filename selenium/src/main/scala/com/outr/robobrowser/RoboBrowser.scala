...
import org.openqa.selenium.devtools.v137.network.model.{RequestWillBeSent, ResponseReceived}
...
//  protected final lazy val _driver: Driver = new EventFiringDecorator[Driver](listener).decorate(createDriver())
  ...
  def save(file: File, context: Context = Context.Browser): Unit = Streamer(content(context), file).sync()
  def screenshot(file: File): Unit = {
    val bytes = capture()
    Streamer(bytes, file).sync()
  }
  ...
  object storage {
    object cookies {
      ...
      def save(file: File): Unit = {
        ...
        Streamer(jsonString, file).sync()
      }

      def load(file: File): Boolean = if (file.isFile) {
        val jsonString = Streamer(file, new mutable.StringBuilder).sync().toString
